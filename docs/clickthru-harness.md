...
| Finding | Status |
|---|---|
| **`thumb.jpg` is never written for a captured sheet** — absent at all 20 steps, including after the dimension persisted, after reload and after export, so the grid card can only show its placeholder. Mechanism traced: `CameraFlow` arms a **3 s** debounce, then unmounts on `onCaptured`, and its cleanup **cancels** the scheduler. `tests/cameraFlow.test.tsx` hand-drives `write()` and so asserts only that `schedule()` was called. | **FIXED — D125.** `CameraFlow` flushes the scheduler **before** `onCaptured`; the re-run reports *card thumbnail rendered: true* at step 03 and `thumb.jpg` (6256 B / 5652 B) present at step 20. Pinned by a test asserting the **order** (a call is not an effect). |
