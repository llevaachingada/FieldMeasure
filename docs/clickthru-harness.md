...
| Finding | Status |
|---|---|
| **`thumb.jpg` is never written for a captured sheet** — absent at all 20 steps, including after the dimension persisted, after reload and after export, so the grid card can only show its placeholder. Mechanism traced: `CameraFlow` arms a **3 s** debounce, then unmounts on `onCaptured`, and its cleanup **cancels** the scheduler. `tests/cameraFlow.test.tsx` hand-drives `write()` and so asserts only that `schedule()` was called. | **D125** — recorded, **not fixed** (`src/**` out of scope for the harness lane) |
...
| Rail renders as the left-most column while `data-rail="right"`, with no `[data-rail='right'] .tool-rail { order }` rule. | **observation only** — the build included a concurrent lane's uncommitted `src/styles.css`; **confirm on a clean tree before treating it as a defect** |
