...
top bar fixed and drag-autoscrolls, and the write lock is bounded. The harness's own findings are fixed:
**D125** (`thumb.jpg` was never written for a captured sheet), **D127** (the rail ignored the handedness
setting), **D128** (a pen barrel press drew). The chrome's and the grid's a11y contracts were **audited by
execution** (**D129** found a 16x48 portrait target and a tab order contradicting UI 14.9; **D130** found
nothing on the grid), and the **arrow-key nudge** is built (**D130**). Capture-blocker rounds D119-D122 are also
closed (the blank failure overlay, the hang, the Web-Lock name collision, the owner's `denied` folder grant).

**Owed, in beta order:** the **hardware pass** (the `[Surface]` rows - nothing emulated promotes them); the
**Offset Nudge Pad** (the glass half of the nudge pair); the **History flyout** (`writeHistorySnapshot` has no
caller); the editor's replace dialog on 13.3:808; `aria-pressed` on the hold; the export owed list; D101's
label halo; **two owner decisions** (the mixed-selection chrome and the 14.9 handedness tab order); and the
content-owner copy.

**Docs hygiene (D131):** D123-D125 had been written **twice** by two concurrent sessions, and D127's heading was
consumed by an insert. Both repaired - one section per number, in order, all 131 unique.
...
**Continuing the same session:** two more lanes on the owed list. **D128** — a pen **barrel press no longer
draws** (the clickthru's step 17 saw `buttons: 2` create a stroke; the router drew for *any* pen contact; only the
tip (`button === 0`) draws now and the editor registers no contact for a non-tip press, so the barrel cannot
draw, ink or pan — the spec's own degrade, since the radial it accelerators is not built; build spec §8.2
amended with three tests that fail pre-fix). **D129** — the editor chrome's **a11y contract audited by
execution** (54 controls, three viewports): four ARROWHEADS options were **16×48 in the portrait bottom dock**
(now a 48 px floor), and the **tab order contradicted UI §14.9** (`TopBar` was the last DOM child with `order: 0`
doing the painting — `order` moves paint, not focus — so the top bar was reached last; now hoisted first). Names,
traps, the focus ring and the rail's roving tabindex verified sound. From that audit's report, fixed here: the
deep sheet's ~48 swatches named their colours with a **bare hex** (now the panel's names where they exist), and
the editor no longer says «Loading projects…» while loading a **sheet**.

**Gates:** tsc 0 · vitest **98 files / 1397 tests** · build 0 (26 precache, 1524.70 KiB) · playwright 5/5.
...
- **The rail-top clipping the owner saw was NOT reproduced** (rail content 758 px in a 908/1068 px box; the
  ...
  visible top.
- **FIXED (D129): the editor no longer says «Loading projects…» while loading a sheet.** `SheetEditor.tsx` was
  rendering `STRINGS.home.loading`; it now renders `editor.loadingSheet` («Loading sheet…», a marked proposal).
  Found by reading every chrome string during the a11y audit (the D110 family).
- **FIXED (D130): the arrow-key nudge is built** — 1 px per press, 10 px with Shift, one undo step per press,
  a held key coalescing into one edit, on the canvas host's own `keydown`. Pinned against a real Konva scene
  (`tests/arrowNudge.browser.test.ts`). **Still owed: the Offset Nudge Pad** — the glass half of the same pair
  (120 px pad, 96 px from the anchor, 0.35× core / 1.0× outer band).
- **The sheets grid's a11y audit found nothing (D130)** after a wave that portaled its card menu out of the
  subtree, turned the page into a scroller and added drag-autoscroll — the portal's focus contract (enter,
  rove, Escape-return, Tab-leaves) and the §14.5 hit-slop separation are now pinned in real layout
  (`tests/gridA11y.browser.test.ts`).
- **Process note:** two lanes could not run this round (the provider account was out of credit — `Insufficient
  Balance`), so the audit, the fix and the tests were done in-session. The lane protocol's *verification*
  discipline is portable; the lanes are not.
- **Owed, a spec question rather than a bug (D129):** UI §14.9 mandates the tab order *rail → canvas → panel*
  regardless of handedness, so for a right-handed user (rail on the **right**) the walk crosses the row
  right→left. A handedness-consistent order would be a **spec change**; the owner should decide.
- **FIXED (D128): the pen barrel button used to draw.** A `buttons: 2` press with freehand active created a
  stroke (the clickthru's step 17); the router now draws only for the tip, and the editor registers no contact
  for a non-tip press, so the barrel can't draw, ink or pan. The re-run reports *"objects 2 → 2 — no object was
  created"* and the exported markup loses exactly that object. **Still `[Surface]`:** whether the real digitiser
  reports `button === 2` at all — H13.
