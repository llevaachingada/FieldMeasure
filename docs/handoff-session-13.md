...
| 19 | **The Project screen (`/p/:projectId`, the sheets grid) does not exist — and no slice owns it** (D88). Build spec §20.5(a) assigns it to slice 1.2; the plan carries it under no slice; 1.2 built **Home's** `ProjectList.tsx` instead; nothing recorded the miss. UI §4.1/§11.9 define it, and its grid's **first two tiles are the add affordances** (`📷 Take photo` + `⬆ Import`); §11.8:667 has capture return *to the sheets grid*. Today `Home → New project` lands in the **Editor**, whose empty state shows the Project screen's copy with only an `Import a photo` button. Missing with it: any sheet list (so no reorder, rename, per-sheet entry, or selection/batch export). **Owner must choose A (build the screen) / B (New project → camera now) / C (fix the empty state only)** — see D88. | new slice |
