...
| H27 | **(D137) Leave the editor right after a real pen stroke:** draw one stroke and tap «Projects» immediately, ten times on ten sheets. Reopen each sheet: every stroke must be there. Then, with the projects folder's grant revoked (site settings), draw and tap «Projects»: the app must stay and say «Couldn't save your last change…», and a second tap must leave. Note any exit that felt slow (> 1 s). | |
| H28 | **(D139) First run on the real Documents folder:** fresh install, tap «Use Documents\FieldMeasure», and in the picker just press «Select Folder» on Documents. Home must show no folders from Documents, and `Documents\FieldMeasure` must exist and hold the first project. Then open the app URL in Firefox (or any non-Chromium browser): the unsupported notice must show, with no dead buttons. | |
| H29 | **(D141/D142) Home and grid on the Surface:** with three projects, Home cards show photo covers and «N sheets · time»; there is no disabled «Open existing folder…» card. On the sheets grid, each card's ⋯ menu and select toggle sit at its right edge, clear of the time/meta line, at 48 px, by touch. | |
