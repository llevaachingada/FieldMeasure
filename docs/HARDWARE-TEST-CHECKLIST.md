...
| **A real touch drag of the Layers grip reorders the row** (D77/F1). | ⚠ **DEFERRED — `fixme`, never a pass, and the product is NOT exonerated.** Root cause executed in session 13 (**D81**): a page that LOADS with an OPFS `FileSystemDirectoryHandle` stored under `fm:projects-root` **kills the renderer** in this Chromium build, so the e2e can never complete first-run. The session-12 story ("`disabled={busy}` never clears") is **withdrawn** — it was a misreading. ⚠ **ALSO UNVERIFIED AND IT DECIDES A PRODUCT QUESTION:** whether a **real on-disk** handle (what a user actually picks) does the same. If it does, this is a **product** defect in `src/settings/projectsRoot.ts`, not a harness limitation. **On hardware: pick a real folder in first-run, then reload — does Home come back?** A CDP-touch browser-project route was attempted in session 13; its test ran and **failed** its reorder assertion, so it was deleted rather than committed. F1's real-touch proof stays **OWED**. |
...
| H21 | **(1.9) C6 — the export memory ceiling on the target device.** Export a 4096×4096 sheet at 3× and confirm it is **refused with the device message**, not attempted. Then find the real ceiling on a Surface Go and record the number. `EXPORT_BITMAP_LIMIT_BYTES` is 512 MB with the arithmetic pinned (D85), but that is a **dev-machine** figure and decides nothing — this row is what sets it. Record in DECISIONS + CHECKPOINTS + the BUILD-LOG `Checkpoints fired:` field. | |
