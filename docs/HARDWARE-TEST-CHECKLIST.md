...
| **A real touch drag of the Layers grip reorders the row** (D77/F1). | ⚠ **DEFERRED — `fixme`, never a pass.** `tests/e2e/layersReorderTouch.spec.ts` drives real CDP touch but stalls in e2e **first-run step 2** (the OPFS stub leaves `disabled={busy}` set), so the editor is never mounted and the drag never runs. The **product is not implicated** — the failure is in the harness, before F1's code. F1's mechanism is covered by a pure test + a browser test using real `elementFromPoint`, but **neither is a real touch**, which is what made F1 invisible. Owed: fix the first-run bootstrap and turn it on. |
