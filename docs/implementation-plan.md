...
8. **Every slice that ships UI carries its own accessibility acceptance** (§19.6): focus order,
   visible focus ring, `aria-label` on every control, 44×44 px minimum target, no keyboard trap.
   Slice 1.10 keeps the themes, the canvas accessible object tree, and the end-to-end audit — it
   is no longer where accessibility *starts*.
...
- `vitest.config.ts` — **two projects in one config**: `node` (pure domain, default) and `jsdom`
  (component tests). Without this, slice 0.3's component tests have no DOM and slice 1.1's pure
  tests pay for one they don't need.
...
9. **(session 4)** `vitest.config.ts` with two projects:
   ```ts
   // test: { projects: [
...
11. **(session 4)** Add the fixtures listed above. Generate them deterministically with a committed
...
12. **(session 4)** Add the two woff2 fonts + `@font-face` (`font-display: swap`) + their OFL text in
...
13. **(session 4)** Manifest `start_url` and `scope` = the exact strings slice 0.0 wrote into
...
Playwright smoke spec. Their job is to prove the **three harnesses run**, not to test anything.
...
- [ ] CI green on push.
- [ ] **(session 4)** `npx vitest run` runs **both** projects (node + jsdom) and both smoke tests pass.
...
- [ ] **(a11y §19.6)** First-run is completable with the keyboard alone; both steps have a visible
      focus ring, an `aria-label` on every control, and 44×44 px minimum targets; the folder-picker
      button is reachable by Tab and announces what it does.
...
- [ ] **(session 4, a11y §19.6)** Every control added in this slice has a visible focus ring and an
      `aria-label`; the read-only and disk-full states are announced, not just coloured.
...
- [ ] **(a11y §19.6)** The canvas container has an accessible name and is not a keyboard trap; zoom
      controls are reachable and labelled.
...
- [ ] **(a11y §19.6)** Shutter, toggles and the review actions are keyboard-operable and labelled;
      the camera-unavailable panel's guidance is text, not an image.
...
2. `ToolRail.tsx`: 14 tools / 6 groups (§11.4) at **56px with 8px gaps** (§14.5). The side is
   **handedness only** (§19 / session-3 finding: rail-side override, bottom rail, pin order and
   Quick Pair are all deferred — "Bottom" contradicts "the tool rail never moves"). Selecting a tool
   whose implementation does not exist yet is a **no-op, not a crash** — that is what lets 1.5–1.8
   land one tool at a time.
...
- [ ] **(a11y §19.6)** The keypad is operable from the hardware keyboard alone; focus is trapped
      ...
      `aria-label`; the live preview is an `aria-live="polite"` region.
...
- **(session 4, F7) pressure actually varies:** build a stroke whose `pressure` array ramps 0.1 → 1.0
  and assert the generated outline's width at the last point is **materially greater** than at the
  first. This is the test that fails if someone re-introduces `p.pressure` (which would make every
  point 0.5 and the two widths identical). Same assertion for `highlight`.
...
- [ ] **(session 4)** A hard-pressed stroke is visibly wider than a light one at the same style
      width, on the device. If they look identical, `pressure` is being read from the wrong place. [Surface]
- [ ] **(a11y §19.6)** Every tool is reachable and operable from the keyboard; the layers panel is a
      proper list with names; focus ring visible on all 56px controls.
...
- [ ] **(a11y §19.6)** Mixed/indeterminate states are announced, not only rendered; disabled controls
      keep an accessible name explaining why they are disabled.
...
- [ ] **(a11y §19.6)** The wizard is fully keyboard-operable; each step is announced; the result
      view's path is selectable text, not an image.
...
- [ ] **(session 4, §19.6)** The **end-to-end** a11y audit passes — per-slice a11y was already gated
      ...
      keyboard-only pass of the core loop, not a first attempt at accessibility.
...
| 0.1 | Scaffold | Airplane-mode reload works; CI green; **all three test harnesses run**; fonts offline | on-device |
...
| 1.5 | Dimension | Keypad truth **and refusal** tables; 4 dims <60 s; labels re-derive; loupe 3.5× at every size | on-device |
| 1.6 | Markup | Ink zoom constancy; highlighter z-below; **pressure visibly varies stroke width** | on-device |
