...
⚠ **CORRECTED (session 22).** The two gestures above contradicted each other — one long-press cannot both reorder the grid and open the card menu — and the build resolves it the only self-consistent way, so this wording now matches the shipped screen: **400 ms long-press = lift and reorder** (UI §11.2:719), and the card menu opens from the per-card **`⋯`** trigger (48 px, over the thumbnail, §14.3's over-canvas treatment). §11.2:716's "editable via long-press → `Rename`" is superseded by the same resolution: **`Rename` is inline** on the card's name row (`Enter` commits, `Esc`/blur cancels, a blank value is a cancel). The card menu also carries **`Move earlier` / `Move later`**, the WCAG 2.1.1 keyboard equivalent of the drag (D115).
