# FieldMeasure — UI/UX Specification
...
**Author:** UI/UX (design preflight). **Status:** Preflight — ready for build.
...
1. **«Which hand do you write with?»** — two big cards (Right / Left), pre-selected from the Windows setting *Bluetooth & devices → Pen & Windows Ink → Choose which hand you write with*. Auto-advance on tap. (This drives rail side, style panel side, loupe offset, keypad side, and handle ordering.)
...
- **Rulers/guides:** off by default. Optional top+left rulers (toggle in overflow) show ft-in ticks when the sheet is calibrated.
- **Palm rejection:** the canvas sets `touch-action: none`, `pointer-events` filtered by `pointerType`, and ignores touch input for 1.2s after a pen `pointerdown` (configurable).
...
| Tool | Controls shown |
|---|---|
| Dimension | Color, Width, Arrowheads (default both), *Precision* (1/16→1"), *Unit format* (ft-in / in / decimal ft), *Calibrate scale* |
...
| Polygon | Color, Width, Line style, Fill, Transparency, *Sides* (3–12), *Close path* |
...
│  │ 7  │ 8  │ 9  │  │ 0  │ ⌫  │ .  │                 │
│  └────┴────┴────┘  └────┴────┴────┘                 │
│  [ ✓ «Use this value» ]   [ «Keep measured 12' 4"» ] │
│  [ ⛓ «Chain: commit & start next from B» ]           │
└──────────────────────────────────────────────────────┘
```

- **Live parse preview** is the point of the whole sheet. It parses continuously and renders the understood value in JetBrains Mono, plus the raw equivalent in the secondary unit. It shows a `--ok` check when the parse is unambiguous, and shows the parser's own reading when it isn't, e.g. typing `12 6` renders `12' 6"` with a ghost `«ft-in»` tag; typing `12' 6 3` renders `12' 6 3/8"` with the `3` underlined as the fraction numerator and a `«← /16»` hint chip that cycles precision on tap.
- **Unit toggles are big and obvious:** `ft` / `in` are 56px toggles. Tapping `in` re-scopes the whole entry to inches (`150 3/8"`). The fraction chips set the rounding precision for the value being entered *and* remember it for the next dimension.
...
- **`«Keep measured …»` (secondary, 56px)** commits the drawn length converted through the sheet's calibration. It shows the actual number it will use, so it is never a mystery button.
...
- **Cancel paths:** `✕`, `Esc`, or tapping the canvas outside the sheet — all three discard the typed value and *keep* the drawn geometry with its measured label. Cancelling must never throw away the stroke the user just drew.
...
**Scale calibration (supporting sub-flow, needed for measured values to mean anything).**
`Long-press Dimension in the rail → «Calibrate scale»`, or the warning chip `«Not calibrated»` on the Style Chip. The user draws a known line, types its true length (`«This line is…»`), and the sheet stores `pxPerFoot`. Until then, the dimension label shows a `--warn` tilde (`≈ 12' 6"`) and the calibrated value is unavailable — the app is honest rather than confidently wrong. Calibration is per sheet, and Copy-scale-to-other-sheets is offered in the confirmation toast.
...
| **Uncalibrated** | Labels prefixed `≈`; `«Use this value»` still available (typed values are always exact). |
...
- **Top-right:** a vertical stack of 56px toggles: torch/flash, grid overlay (rule of thirds), level indicator on/off, camera flip (front/rear — default **rear**), and resolution (`«High 12.3 MP»` / `«Fast 4 MP»`, because a field tablet may need throughput over pixels).
...
- Bottom row: `↺ «Retake»` (56px, secondary) · `↻` rotate · `✨ «Auto-enhance»` (toggle, applies a levels/contrast fix — genuinely useful for reading faded construction markings) · `✓ «Use photo»` (64px, `--hi`, primary).
...
- **Secondary row of cards** at the bottom of the grid: `«Open existing folder…»` (dashed `--g700` border, 56px icon, opens `showDirectoryPicker`) and `«Import a project»` (for a `.fieldmeasure` bundle or a folder dropped in). Both must look like cards so they live in the same scan path.
- **Card interactions:** tap = open Project. Long-press = context menu (`Rename`, `Show in Explorer` (via the OS), `Reveal path`, `Export all`, `Remove from this list` (never deletes files — copy: `«Removes it from this list only. Files on disk are untouched.»`), `Delete files…` (hold-to-confirm, with the exact path shown in mono)).
...
- **Top bar (56px):** `‹ Projects` · project name (inline editable, Archivo 700 20px) · `«12 sheets»` count · right: `⌗` (nothing here), **`⇧ «Export»`** (primary), `⋯` (Project settings, Rename, Show in Explorer, Copy path, Duplicate project, Delete project).
...
- **Swipe/tap-hold on a card** opens the card menu: `Open`, `Rename`, `Duplicate`, `Rotate`, `Replace photo`, `Delete`.
...
- **PDF options:** page size `«Fit to photo»` (recommended, one page per sheet at the photo's aspect) — do not offer paper sizes as the default; field crews want the photo, not Letter. Quality: `1× / 2× / 3×` with estimated MB. Checkbox `«Flatten markup into the image»` (default ON — keeps dimension labels crisp because text is rendered at the chosen multiplier, and makes the PDF universally readable) vs OFF (`«Keep pages as image + vector overlay»`, larger, editable in Acrobat). Checkbox `«Include a dimensions summary page»` (default OFF; a table of every dimension on every sheet, in mono, with sheet name and value — a genuinely valuable report artifact). Checkbox `«Include sheet names in pages»`.
...
| Changing a sheet's base photo | No (markup is kept, photo is not) | Dialog + hold-to-confirm. |
...
8. **Handedness.** Sourced from the Windows pen setting at first run, overridable in Settings. It mirrors: rail side, style panel side, loupe offset direction, keypad numeric layout (digits cluster toward the writing hand), mini-toolbar anchor, and the rotation of the align-guide priority. Changing it re-renders in place without losing canvas state.
...
10. **Uncalibrated dimensions show `≈`** and are honest about it. (§8.1)
...
| Export | `«Select at least one sheet»`, primary disabled | Per-file progress rows | Per-file errors with Retry | Result view with Copy path / Open folder |
